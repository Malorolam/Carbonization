...
//				PacketDispatcher.sendPacketToAllAround(xCoord, yCoord, zCoord, 64, worldObj.provider.dimensionId, getDescriptionPacket());
	...
	public boolean handleUpgrade(boolean action)
	{
		...
		{
			if(UtilReference.getItemBurnTime(upgradeStacks[2],1,false)>0)//it's a bit of fuel
			{
	...
	public boolean addFuel(ItemStack item)
	{
		if(item == null)
			return false;
		int time = UtilReference.getItemBurnTime(item,(int)(this.maxFuelCapacity-this.fuelTank),true);
		if(time <= 0)
			return false;
