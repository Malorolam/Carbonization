...
public class MultiBlockMatcher {
	...
	public MultiBlockMatcher(int xdiff, int ydiff, int zdiff)
	{
		...
		pattern = new int[xdiff][ydiff][zdiff];
		//Initialize the pattern with "air"
		buildSolid(0,0,0,xdiff-1,ydiff-1,zdiff-1,0);
	}
	 ...
	 */
	public boolean comparePattern(int[][][] test_pattern)
	{
		...
		for(int i = 0; i<pattern.length; i++)
			for(int j = 0; j<pattern[0].length; j++)
				...
				{
					if(pattern[i][j][k] != test_pattern[i][j][k])
						return false;
				}
		return true;
	}
	 ...
	 */
	public boolean buildSolid(int minX, int minY, int minZ, int maxX, int maxY, int maxZ, int blockID)
	{	
		...
		{
			System.err.println("Build Solid process failed: Maximum values exceed pattern size.");
			return false;
		}
	 ...
	 */
	public boolean buildComplexSolid(Object[] shape)
	{
		return false;
	}
	 ...
	 */
	public boolean buildHollowSolid(int minX, int minY, int minZ, int maxX, int maxY, int maxZ, int blockID, int wallThickness)
	{			
		//make sure none of the min values is less than 0, since that makes arrays explode
		if(minX < 0 || minY < 0 || minZ < 0)
		{
		...
		if(maxX >= xdiff || maxY >= ydiff || maxZ >= zdiff)
		{
			System.err.println("Build Hollow Solid process failed: Maximum values exceed pattern size.");
			return false;
		}
		...
		//Exterior shape
		success=buildSolid(minX, minY, minZ, maxX, maxY, maxZ, blockID);
		if(success==false)
		{
		...
		//interior space
		success=buildSolid(minX+wallThickness, minY+wallThickness, minZ+wallThickness, maxX-wallThickness, maxY-wallThickness, maxZ-wallThickness, 0);
		if(success==false)
		{
			System.err.println("Build Hollow Solid process failed: Previous process failed.");
			return false;
	 ...
	 */
	public boolean setBlock(int i, int j, int k, int blockID)
	{
