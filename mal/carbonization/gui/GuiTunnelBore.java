...
import mal.carbonization.containers.ContainerTunnelBore;
...
public class GuiTunnelBore extends GuiContainer {
	...
	{
		return (int) (bore.getFuelStack()*i/(bore.getMaxFuel()+1));
	}
	...
	public void drawScreen(int par1, int par2, float par3)
	{
		...
		if(UtilReference.isPointInRegion(152, 11, 16, 52, par1, par2, guiLeft, guiTop))
		{
			...
			list.add(ColorReference.DARKGREY.getCode() + bore.getFuelStack()+"/"+bore.getMaxFuel()+"FP");
			
	        this.drawHoveringText(list, par1, par2, fontRendererObj);
		}
		
		//see if the mouse is over the information
		if(UtilReference.isPointInRegion(8, 30, 72, 22, par1, par2, guiLeft, guiTop))
		{
			ArrayList list = new ArrayList();
