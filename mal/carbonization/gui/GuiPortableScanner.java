...
import mal.carbonization.containers.ContainerPortableScanner;
...
public class GuiPortableScanner extends GuiContainer {
	...
	public void drawScreen(int par1, int par2, float par3)
	{
		...
		if(UtilReference.isPointInRegion(152, 11, 16, 52, par1, par2, guiLeft, guiTop))
		{
			...
			list.add(ColorReference.DARKGREY.getCode() + String.format("%.2f", ((ItemPortableScanner) scanner.getStack().getItem()).getFuelValue(scanner.getStack()))+"/6400"+"FT");
			
	        this.drawHoveringText(list, par1, par2, fontRendererObj);
		}
		
		//see if the mouse is over the information
		if(UtilReference.isPointInRegion(8, 30, 72, 32, par1, par2, guiLeft, guiTop))
		{
			ArrayList list = new ArrayList();
		...
		if(UtilReference.isPointInRegion(8, 10, 16, 16, par1, par2, guiLeft, guiTop) && scanner.getItemInSlot(0)==null)
		{
			ArrayList list = new ArrayList();
		...
		//see if the mouse is over the upgrade slots
		if(UtilReference.isPointInRegion(26, 10, 16, 16, par1, par2, guiLeft, guiTop) && scanner.getItemInSlot(1)==null)
		{
			ArrayList list = new ArrayList();
		...
		//see if the mouse is over the upgrade slots
		if(UtilReference.isPointInRegion(44, 10, 16, 16, par1, par2, guiLeft, guiTop) && scanner.getItemInSlot(2)==null)
		{
			ArrayList list = new ArrayList();
		...
		//see if the mouse is over the upgrade slots
		if(UtilReference.isPointInRegion(134, 10, 16, 16, par1, par2, guiLeft, guiTop) && scanner.getItemInSlot(3)==null)
		{
			ArrayList list = new ArrayList();
		...
		//see if the mouse is over the upgrade slots
		if(UtilReference.isPointInRegion(98, 10, 16, 16, par1, par2, guiLeft, guiTop) && scanner.getItemInSlot(4)==null)
		{
			ArrayList list = new ArrayList();
