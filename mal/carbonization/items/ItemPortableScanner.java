...
public class ItemPortableScanner extends Item {
	...
	public static final int MAXFUEL = 3200;
	
	public ItemPortableScanner(int par1) {
		super(par1);
	...
	public static ItemStack[] getInventory(ItemStack is)
	{
		...
		NBTTagList items = is.stackTagCompound.getTagList("inventory");
		...
		for (int i = 0; i < items.tagCount(); ++i)
		{
			...
			{
				ItemStack ii = new ItemStack(0,0,0).loadItemStackFromNBT(item);
				inventory[i] = ii;
			}
		}
...
/*		ItemStack[] stack = new ItemStack[3];
	...
	public static double getFuelUsage(ItemStack is)
	{
		...
		double baseUse = carbonization.BASEAUTOCRAFTFUEL/66;
	...
	public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer player)
	{
		...
		{
			player.openGui(carbonization.instance, 4, world, 0, 0, 0);
		}
	...
	public boolean onItemUse(ItemStack is, EntityPlayer player, World world, int x, int y, int z, int side, float px, float py, float pz)
	{
		if(!world.isRemote)
		{
			...
			{
				player.addChatMessage("Insufficient Fuel: " + String.format("%.2f",fuelValue) + "/" + String.format("%.2f",fuelUse));
				return false;
			}
			else
			{
